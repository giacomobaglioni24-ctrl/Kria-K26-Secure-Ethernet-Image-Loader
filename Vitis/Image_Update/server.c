...
static void PrintAppHeader(struct tcp_pcb *Tpcb);
...
static void PrintAppHeader(struct tcp_pcb *Tpcb)
{
    ...
    sprintf(WriteBuffer, "\r\n\r\n---------- Ethernet Server DDR DMA ----------\r\n\r\nBoard IP: 192.168.1.10\r\nNetmask 255.255.255.0\r\nGetaway: 192.168.1.1\r\n\r\nComands:\r\nGET/boot_img_status/\r\nGET/flash_erase_imgA/\r\nGET/flash_erase_imgB/\r\nPOST/Upload_Img_A/XXXXXXXXBOOT.BIN\r\nPOST/Upload_Img_B/XXXXXXXXBOOT.BIN\r\n\r\nWhere 'XXXXXXXX' are 8 bytes to communicate BOOT.BIN's lenght in byte, so you have to convert it in exadecimal\r\n\r\n");
...
static err_t RecvCallback(void *arg, struct tcp_pcb *Tpcb, struct pbuf *p, err_t err)
{
	...
	if(TcpArg->PktCount == 1) {
		...
		Status = ProcessReq(Tpcb, p->payload, p->len);
		...
		Status = ProcessAdditionalPayload(Tpcb, p->payload, p->len);
		if ( Status != XST_SUCCESS ) {
			ResetTcpArgCounters(Tpcb);
			tcp_close(Tpcb);
    	}		
